<template name="note">
  ...
  <div class="group text-black p-2 m-2 rounded-xl flex flex-col " v-for="(note, index) in notes" :key="index" v-bind:class="[note.checked ? 'bg-slate-500' : 'bg-slate-300']">
    ...
    <li class="note_text rounded-md h-full" v-if="note.type == 'note'">{{ note.text }}</li>
      <li class="flex items-center" v-if="note.type == 'list'" v-for="(line, index) in lines" :key="index"><button class="check_button filter-blue bg-[url('./assets/svg/chevron-forward-circle-outline.svg')] w-5 h-5" v-on:click="changeNote(index, 'toggle_row')"></button>{{ line }}</li>
    </ul>
